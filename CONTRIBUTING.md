...
3. **Use the Unified Items Format**
   ...
   > 📖 **For the complete grammar shape** — including the `performance`
   > object for video clips, `metadata.youtube_video_id` conventions,
   > the composite-items / emergence pattern, and the three common
   > mistakes that prevent grammars from loading — see
   > [GRAMMAR_FORMAT.md](GRAMMAR_FORMAT.md). It is the single source of
   > truth.
