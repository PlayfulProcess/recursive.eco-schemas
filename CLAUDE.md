...
> 📖 **Canonical reference: [GRAMMAR_FORMAT.md](GRAMMAR_FORMAT.md).**
> Read that first if you are creating a new grammar or fixing one
> that won't load. The summary below is a quick reference only.
...
```json
{
  ...
  "grammar_type": "custom|tarot|iching|astrology|sequence|course|prompt|birthchart|altar|music",
...
> 📖 **For the complete canonical grammar format**, including the
> `performance` object (YouTube clip cropping), `metadata.youtube_video_id`
> conventions, and the three common mistakes that prevent grammars
> from loading, see [GRAMMAR_FORMAT.md](GRAMMAR_FORMAT.md). The list
> below is a quick reference only.

- `tarot` — Oracle/card-draw systems (Rider-Waite, Jungian Archetypes)
- `iching` — I Ching hexagram readings (must have 64 items with `lines`)
- `astrology` — Astrological interpretation (Western or Vedic)
- `sequence` — Ordered content (video playlists, story sequences) — usually
  paired with per-item `performance` objects for timestamped video clips
- `course` — Step-by-step learning sequences
...
- `custom` — Everything else (texts, mythology, kids content, poetry, journaling)

> ⚠️ **`"performance"` is NOT a valid `grammar_type`.** It is a viewer
> mode, not a grammar type. Use `"sequence"` for video playlists; the
> per-item `performance` object carries the clip details.

## Meta-Grammars & Reference Items (a grammar of grammars)

A grammar's items can **point at other grammars** instead of holding content, which
makes a "grammar of grammars" possible. On a `UnifiedItem`:

- `item_type: "reference"` + `ref_document_id` — the item opens another
  `user_document` (the viewer renders it as a `/play?id=…` link).
- `ref_preview: "grammar" | "study" | "altar"` — how the target opens.
- `grammars: [id, …]` — multi-link to connected grammars.

At the grammar root, `default_preview: "tree"` opens the recursive.eco **tree-viewer**
(`pages/tree-viewer.html?type=custom&id=…`), which draws the L1/L2/L3 `composite_of`
emergence as a tree with connecting edges.

**Live example — the genealogy of tarot:** `grammars/tree-of-tarot/grammar.json` is a
meta-grammar whose **L1** leaves are the tarot decks, **L2** are the branches (Michael
Dummett's A/B/C trump-orders + the occult reframing + sui-generis offshoots), and
**L3** is the root. It sets `default_preview: "tree"`. After the decks are imported to
Supabase, each leaf's `ref_document_id` is set so clicking it opens that deck.

This is recursion **by reference** — items are NOT merged into the grammar type.
(Full item/grammar type-unification is deliberately NOT done — too deep a refactor over
the live grammars for too little gain; see `plan/tarot-roadmap-and-supabase-log.md`
§6f.) Full field reference: `GRAMMAR_FORMAT.md` → "Reference items & meta-grammars".
