...
## Grammar JSON format — the canonical reference

**[📖 GRAMMAR_FORMAT.md](GRAMMAR_FORMAT.md)** documents the complete
shape of a grammar JSON file. Read it first if:

- You are writing a grammar by hand or with AI assistance
- Your grammar won't load in the viewer or editor
- You are working with timestamped video clips (`performance` object)
- You are unsure which `grammar_type` to use
- You see a top-level `emergences[]` array somewhere and want to know
  why it doesn't work

The other docs in this repo (`CLAUDE.md`, `CONTRIBUTING.md`,
`schemas/SCHEMAS_GUIDE.md`) describe pieces of the format; that one
